...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func retweet(_tweetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let url = "https://api.twitter.com/1.1/statuses/retweet/\(_tweetId).json"
        TwitterAPICaller.client?.post(url, parameters: ["id": _tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
    }
    func unRetweet(_tweetId:Int, success: @escaping () -> (), failure: @escaping (Error) -> ()){
        let url = "https://api.twitter.com/1.1/statuses/unretweet/\(_tweetId).json"
        TwitterAPICaller.client?.post(url, parameters: ["id": _tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
